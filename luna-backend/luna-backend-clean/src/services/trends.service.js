...
const identifyRecurringPatterns = (cycles, readings) => {
  ...
  if (regularity.irregular) {
    patterns.push(`Irregular cycles detected over ${cycles.length} recorded cycles (avg ±${regularity.stdDev} days).`);
  }

  const poorSleepDays = readings.filter(r => (r.sleepHours || 7) < 6).length;
  if (poorSleepDays > readings.length * 0.4) {
    patterns.push('Recurring poor sleep across multiple cycles detected. This may be linked to hormonal changes.');
  }

  const highTempDays = readings.filter(r => (r.temperature || 36.6) > 37.0).length;
  if (highTempDays > 3) {
    patterns.push(`Elevated temperature occurred on ${highTempDays} days in the recorded period.`);
  }

  if (patterns.length === 0) {
    patterns.push('No recurring irregularities detected. Your cycle patterns appear healthy.');
  }
  return patterns;
};
